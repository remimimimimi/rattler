...
use run_exports_extractor::{
    DumpPackageCacheReporter, RunExportExtractor, RunExportExtractorError, RunExportReporter,
};
use subdir::Subdir;
    ...
    pub async fn ensure_run_exports(
        ...
        for (pkg_idx, pkg) in records.iter().enumerate() {
            ...
            let extractor = RunExportExtractor::default()
                ...
                .with_package_cache(self.inner.package_cache.clone(), DumpPackageCacheReporter);
...
mod test {
    ...
    fn run_exports_missing(records: &Vec<RepoDataRecord>) -> bool {
        records
            .iter()
            .any(|rr| rr.package_record.run_exports.is_none())
    }

    fn run_exports_in_place(records: &Vec<RepoDataRecord>) -> bool {
        records
            .iter()
